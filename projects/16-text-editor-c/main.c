...
void abAppend(struct abuf *ab, const char *s, int len) {
  char *new_buf = realloc(ab->b, (size_t)ab->len + (size_t)len);
  if (new_buf == NULL) return;
  memcpy(&new_buf[ab->len], s, (size_t)len);
  ab->b = new_buf;
  ab->len += len;
}
...
void editorSave() {
  if (E.filename == NULL) {
    editorSetStatusMessage("Save aborted: open with filename argument");
    return;
  }
  ...
  free(buf);
  editorSetStatusMessage("Save error: %s", strerror(errno));
}
  ...
  for (y = 0; y < E.screenrows; y++) {
    ...
    if (filerow >= E.numrows) {
      if (E.numrows == 0 && y == E.screenrows / 3) {
        char welcome[80];
        int welcomelen = snprintf(welcome, sizeof(welcome), "zero-editor -- CTRL-S = save | CTRL-Q = quit");
